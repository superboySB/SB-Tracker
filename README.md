...
开放环境下基于人机交互的UAV Tracker，部署在笔记本电脑上（后续应该还要进一步到Jetson Orin板载上）。基本思路是：我们会先用yolo-world给用户检测物体类别（基于CLIP的开集检测，类别可以自己给，具有一定的具身性），然后用户用鼠标点击要跟踪的物体，可以即刻跟踪物体。由于同时加载了端侧的Segment Anything模型，用户既可以点击指定类别框内的物体，也可以临时点击视野内没有标记检测框的任意物体，都可以尝试进行跟踪，`r`为重置，`q`为退出。
